...
@pytest.fixture
def mock_github_json() -> dict[str, Any]:
...
class MockResponse:
    ...
    def iter_content(self, _chunk_size: int = 1024) -> Generator[bytes, None, None]:
        yield self.text.encode("utf-8")


def test_get_class_classifier_moddb() -> None:
...
def test_get_class_classifier_github() -> None:
...
def test_get_class_classifier_other() -> None:
...
def test_source_moddb_from_url(monkeypatch: MonkeyPatch, mock_moddb_html: str) -> None:
    ...
    assert source.added == datetime(2023, 1, 1, 12, 0, tzinfo=UTC)
    assert source.updated == datetime(2023, 1, 2, 12, 0, tzinfo=UTC)
    assert source.size == "123 MB"
    assert source.checksum == "abcdef1234567890"
    assert source.download_url == "/downloads/start/12345"


def test_source_github_from_url(monkeypatch: MonkeyPatch, mock_github_json: dict[str, Any]) -> None:
    ...
    assert source.added == datetime(2023, 1, 1, 0, 0, tzinfo=UTC)
    assert source.updated == datetime(2023, 1, 2, 0, 0, tzinfo=UTC)
    assert source.size == "1234kb"
    assert source.download_url == "https://api.github.com/repos/test/Test-Repo/zipball"


def test_source_base_check_if_file_exists(tmp_path: Path) -> None:
    ...
    source = SourceBase(
        ...
        added=fixed_dt,
        updated=fixed_dt,
        checksum="27565f9a57c128674736aa644012ce67",
        size="1KB",
        url="",
        download_url="",
    )
...
def test_source_moddb_from_dict() -> None:
    ...
    data = {
        ...
        "updated": "2023-01-03T00:00:00+00:00",
        "size": "1 MB",
        "checksum": "12345",
        "url": "http://moddb.com/test",
        "download_url": "http://moddb.com/download",
    }
    source = SourceModdb.from_dict(data)
    assert source.title == "Test Mod"
    assert source.folders == ["folder1", "folder2"]
    assert source.updated == datetime(2023, 1, 3, 0, 0, tzinfo=UTC)


def test_source_github_from_dict() -> None:
    ...
    data = {
        ...
        "size": "2 MB",
        "checksum": "",
        "url": "http://github.com/api/test",
        "html_url": "http://github.com/test",
        "download_url": "http://github.com/zipball",
    }
