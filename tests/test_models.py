...
def test_mod_model_data(mock_mod_settings):
...
def test_mod_model_set_data(mock_mod_settings):
...
def test_mod_model_flags(mock_mod_settings):
...
def test_source_model_init(mock_source_list):
    """Tests the initialization of SourceModel."""
    model = SourceModel(sources=mock_source_list)
    assert model.rowCount() == 2
    assert model.columnCount() == 6

def test_source_model_header_data(mock_source_list):
...
def test_source_model_data(mock_source_list):
